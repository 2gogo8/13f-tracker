...
# 5 badges
tickers = ['GOOGL σ-2.4', 'PLTR σ-1.7', 'RKLB σ-0.6', 'AMD σ-2.2', 'ARES σ-1.6']
for i, t in enumerate(tickers):
    x = 1.8 + i * 2.0
    card = add_card(s, x, 4.2, 1.8, 0.5)
    add_text(s, x, 4.25, 1.8, 0.4, t, 13, GOLD, bold=True, align=PP_ALIGN.CENTER)
...
def make_company_section(ticker, name, subtitle, price_str, sigma, data):
    ...
    s = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(s)
    add_text(s, 0.8, 0.4, 5, 0.6, f"{ticker} 公司概況", 30, GOLD, bold=True)
    ...
    s = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(s)
    add_text(s, 0.8, 0.4, 5, 0.6, f"{ticker} 營收結構", 30, GOLD, bold=True)
    add_gold_line(s, 0.8, 1.0, 1.5)
    ...
    s = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(s)
    add_text(s, 0.8, 0.4, 8, 0.6, f"{ticker} 資本支出應用場景", 30, GOLD, bold=True)
    add_gold_line(s, 0.8, 1.0, 1.5)
    ...
    s = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(s)
    add_text(s, 0.8, 0.4, 8, 0.6, f"{ticker} 2026 重大動向與消息", 30, GOLD, bold=True)
    add_gold_line(s, 0.8, 1.0, 1.5)
    ...
    s = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(s)
    add_text(s, 0.8, 0.4, 8, 0.6, f"{ticker} 2026 催化劑時間表", 30, GOLD, bold=True)
    add_gold_line(s, 0.8, 1.0, 1.5)
    ...
    s = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(s)
    add_text(s, 0.8, 0.4, 8, 0.6, f"{ticker} 投資結論", 30, GOLD, bold=True)
    add_gold_line(s, 0.8, 1.0, 1.5)
