...
function assignGroup(slope: number, benchSlope: number): string {
  // When bench is negative, use bench/5 as threshold (more lenient but meaningful)
  // When bench is positive, use bench*10 as the explosive threshold
  const explosiveThreshold = benchSlope >= 0 ? benchSlope * 10 : benchSlope / 5;
  if (slope >= explosiveThreshold) return '⚡爆賺';
  ...
  try {
    ...
    if (fs.existsSync(priceCachePath) && date1 && date2) {
      ...
      return NextResponse.json({
        ...
        explosive_threshold: Math.round((benchSlope >= 0 ? benchSlope * 10 : benchSlope / 5) * 100) / 100,
