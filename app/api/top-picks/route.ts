...
// Only scan 10 mega-caps to stay within Vercel Hobby 10s limit
const SYMBOLS = [
  'AAPL','MSFT','GOOGL','AMZN','NVDA','META','TSLA','NFLX','CRM','PYPL',
];
...
export async function GET() {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      SYMBOLS.map(async (symbol): Promise<TopPick | null> => {
        const [histRes, quoteRes] = await Promise.all([
          fetch(`https://financialmodelingprep.com/stable/historical-price-eod/full?symbol=${symbol}&from=${fromStr}&to=${toStr}&apikey=${FMP_KEY}`, { signal: AbortSignal.timeout(6000) }),
          fetch(`https://financialmodelingprep.com/stable/quote?symbol=${symbol}&apikey=${FMP_KEY}`, { signal: AbortSignal.timeout(6000) }),
        ]);
        ...
        if (!quote || hist.length < 21) return null;
